-- Helpdesk tickets: create → manager approve → assigned person/dept handles

DO $$ BEGIN
  CREATE TYPE public.helpdesk_category AS ENUM (
    'incident', 'request', 'question', 'access', 'other'
  );
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE TYPE public.helpdesk_priority AS ENUM (
...
DO $$ BEGIN
  CREATE TYPE public.helpdesk_status AS ENUM (
    'pending_manager', 'open', 'in_progress', 'resolved', 'closed', 'rejected', 'cancelled'
  );
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
CREATE TABLE IF NOT EXISTS public.helpdesk_tickets (
  ...
  title text NOT NULL,
  description text,
  category public.helpdesk_category NOT NULL DEFAULT 'request',
  priority public.helpdesk_priority NOT NULL DEFAULT 'medium',
  status public.helpdesk_status NOT NULL DEFAULT 'pending_manager',
  ...
  CONSTRAINT helpdesk_tickets_title_len CHECK (char_length(trim(title)) BETWEEN 3 AND 200),
  CONSTRAINT helpdesk_tickets_assignee_xor CHECK (
    (assignee_employee_id IS NOT NULL AND assignee_department_id IS NULL)
    OR (assignee_employee_id IS NULL AND assignee_department_id IS NOT NULL)
  )
);

CREATE UNIQUE INDEX IF NOT EXISTS helpdesk_tickets_company_number_uidx
  ON public.helpdesk_tickets (company_id, ticket_number)
...
CREATE INDEX IF NOT EXISTS helpdesk_tickets_assignee_emp_idx
  ON public.helpdesk_tickets (assignee_employee_id)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS helpdesk_tickets_assignee_dept_idx
  ON public.helpdesk_tickets (assignee_department_id)
  WHERE deleted_at IS NULL;

ALTER TABLE public.helpdesk_tickets ENABLE ROW LEVEL SECURITY;

CREATE OR REPLACE FUNCTION public.is_helpdesk_assignee(p_ticket public.helpdesk_tickets)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (
    ...
    WHERE e.user_id = auth.uid()
      ...
      AND (
        e.id = p_ticket.assignee_employee_id
        OR (
          p_ticket.assignee_department_id IS NOT NULL
          AND e.department_id = p_ticket.assignee_department_id
        )
      )
  );
$$;
...
CREATE POLICY helpdesk_tickets_select
  ...
  USING (
    ...
    AND (
      ...
      OR public.manages_employee(requester_employee_id)
      OR EXISTS (
        SELECT 1 FROM public.employees e
...
BEGIN
  FOR v_uid IN
    ...
    WHERE e.department_id = p_department_id
      ...
      AND (p_exclude_user_id IS NULL OR e.user_id <> p_exclude_user_id)
  LOOP
    PERFORM public.notify_user(v_uid, p_company_id, p_kind, p_title, p_body, p_href);
  END LOOP;
END;
$$;

CREATE OR REPLACE FUNCTION public.notify_helpdesk_assignees(p_ticket public.helpdesk_tickets)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DECLARE
  v_uid uuid;
BEGIN
  IF p_ticket.assignee_employee_id IS NOT NULL THEN
    SELECT e.user_id INTO v_uid
    FROM public.employees e
    WHERE e.id = p_ticket.assignee_employee_id AND e.deleted_at IS NULL;
    PERFORM public.notify_user(
      v_uid, p_ticket.company_id, 'helpdesk',
      'Nouveau ticket assigné: ' || p_ticket.ticket_number,
      p_ticket.title,
      '/helpdesk'
    );
  ELSIF p_ticket.assignee_department_id IS NOT NULL THEN
    PERFORM public.notify_department_members(
      p_ticket.assignee_department_id,
      p_ticket.company_id,
      'helpdesk',
      'Nouveau ticket pour votre département: ' || p_ticket.ticket_number,
      p_ticket.title,
      '/helpdesk'
    );
  END IF;
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Fiche employé introuvable — seul un collaborateur peut ouvrir un ticket';
  END IF;
  ...
  -- Auto-open if requester is their dept manager, or no manager configured (RH fallback notify)
  IF v_req.department_id IS NOT NULL THEN
  ...
  INSERT INTO public.helpdesk_tickets (
    ...
    requester_employee_id, requester_department_id,
...
BEGIN
  ...
  v_req_uid := v_req.user_id;
  v_is_req := v_req.user_id = v_uid;
  v_can_hr := public.can_manage_company(v_t.company_id) OR public.is_platform_admin();
  v_can_mgr := public.manages_employee(v_t.requester_employee_id) OR v_can_hr;
...
GRANT EXECUTE ON FUNCTION public.is_helpdesk_assignee(public.helpdesk_tickets) TO authenticated;
GRANT EXECUTE ON FUNCTION public.notify_department_members(uuid, uuid, text, text, text, text, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.notify_helpdesk_assignees(public.helpdesk_tickets) TO authenticated;
GRANT EXECUTE ON FUNCTION public.create_helpdesk_ticket(uuid, text, text, public.helpdesk_category, public.helpdesk_priority, uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.transition_helpdesk_ticket(uuid, text, text) TO authenticated;
